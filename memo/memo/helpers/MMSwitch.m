...
typedef enum {
  MMSwitchThumbJustifyLeft,
  MMSwitchThumbJustifyRight
} MMSwitchThumbJustify;

@interface MMSwitchThumb : UIView
@property (nonatomic, assign) BOOL isTracking;


-(void) growThumbWithJustification:(MMSwitchThumbJustify) justification;
-(void) shrinkThumbWithJustification:(MMSwitchThumbJustify) justification;
@end

@interface MMSwitchTrack : UIView
...
@property (nonatomic, strong) MMSwitchTrack* track;
@property (nonatomic, strong) MMSwitchThumb* thumb;
...
-(void) configureSwitch {
  ...
  // Initialization code
  if (!_track) {
    _track = [[MMSwitchTrack alloc] initWithFrame: self.bounds
  ...
  if (!_thumb) {
    _thumb = [[MMSwitchThumb alloc] initWithFrame:CGRectMake(kThumbOffset, kThumbOffset, self.bounds.size.height - 2 * kThumbOffset, self.bounds.size.height - 2 * kThumbOffset)];
...
- (void)setOn:(BOOL)on
     animated:(BOOL)animated {
  [self setOn:on animated:animated shouldCallback:YES];
}

- (void)setOn:(BOOL)on animated:(BOOL)animated shouldCallback:(BOOL)shouldCallback {
  ...
  [self setOn: on shouldCallback:shouldCallback];
}

- (void) setOn:(BOOL)on {
  [self setOn:on shouldCallback:YES];
}

- (void) setOn:(BOOL)on shouldCallback:(BOOL)shouldCallback {
  ...
  if (self.didChangeHandler && shouldCallback) {
...
-(void) setThumbIsTracking:(BOOL)isTracking {
  if (isTracking) {
    //Grow
    [self.thumb growThumbWithJustification: self.isOn ? MMSwitchThumbJustifyRight : MMSwitchThumbJustifyLeft];
  }
  else {
    //Shrink
    [self.thumb shrinkThumbWithJustification: self.isOn ? MMSwitchThumbJustifyRight : MMSwitchThumbJustifyLeft];
  }
...
-(void) growThumbWithJustification:(MMSwitchThumbJustify) justification {
  ...
  if (justification == MMSwitchThumbJustifyRight) {
